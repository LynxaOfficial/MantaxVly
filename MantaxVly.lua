-- [[ MANTΛX VLY v7.0 - BLOX FRUITS PREMIUM ]]
...
local function sendWebhook(title, msg)
    ...
    pcall(function()
        syn.request({Url = webhookURL, Method = "POST", Headers = {["Content-Type"] = "application/json"}, Body = HttpService:JSONEncode({embeds = {{title = title, description = msg, color = 16777215, footer = {text = "MantaxVly | "..os.date("%H:%M:%S")}}}})})
    end)
end
    ...
    while task.wait(0.2) do
        if settings.autoFarm then
            pcall(function()
                ...
                if quest then
                    for _, enemy in pairs(Workspace.Enemies:GetChildren()) do
                        if enemy.Name == quest.name and isAlive(enemy) then
                            equipBest()
        ...
        if settings.autoBoss then
            pcall(function()
                ...
                for _, bossName in ipairs(liveBosses) do
                    if bossName == settings.selectedBoss then
                        for _, enemy in pairs(Workspace.Enemies:GetChildren()) do
    ...
    while task.wait(0.5) do
        if settings.autoRaid then
            pcall(function()
                ...
                if settings.autoStartRaid then
                    ReplicatedStorage.Remotes.StartRaid:FireServer(settings.selectedRaid:gsub(" Raid",""))
                end
                for _, enemy in pairs(Workspace.Enemies:GetChildren()) do
                    if enemy.Name:find(settings.selectedRaid:gsub(" Raid","")) and isAlive(enemy) then
                        equipBest()
                        repeat
                            tween(enemy.HumanoidRootPart.Position)
                            attack(enemy)
                    ...
                    if tp ~= Player and tp.Character and isAlive(tp.Character) then
                        ...
                        if math.abs(ml - tl) <= 100 and (tp.Data.Bounty or 0) > 0 then
                            ...
                            repeat
                                ...
                                attack(tp.Character)
                                task.wait(0.05)
                            until not settings.bountyHunter or not tp.Character or not isAlive(tp.Character)
                        end
                    end
                end
            end)
...
coroutine.wrap(function()
    while task.wait(20) do
        if settings.autoFarm or settings.bountyHunter then
            pcall(function()
        ...
        pcall(function()
            for _, item in pairs(Workspace:GetChildren()) do
                if item:IsA("Tool") and item.Name:find("Fruit") and not spawnedFruits[item] then
                    ...
                    sendWebhook("FRUIT SPAWNED", item.Name:gsub(" Fruit",""):gsub("-Fruit","") .. " spawned at " .. os.date("%H:%M:%S"))
                end
            end
        end)
    end
...
coroutine.wrap(function()
    ...
    while frame.Parent do
        pcall(function()
            ...
            local liveBossCount = #getLiveBosses()
            info.Text = string.format("Level: <b>%s</b>\nBeli: <b>%s</b>\nBounty: <b>%s</b>\nFruit: <b>%s</b>\nRace: <b>%s</b>\nHP: <b>%d/%d</b>\nLive Bosses: <b>%d</b>", lv, beli, bounty, fruit, race, hp, mhp, liveBossCount)
        end)
        task.wait(2)
    end
end)()
...
coroutine.wrap(function()
    ...
    task.delay(2.5, function() gui:Destroy() notify("MANTΛX VLY", "Script ready! Sea "..SEA, 3) sendWebhook("SCRIPT LOADED", "Sea "..SEA.." | Level "..getLevel()) end)
end)()
...
coroutine.wrap(function()
    ...
    btn.MouseButton1Click:Connect(function()
        open = not open
        if open then TweenService:Create(main, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {Size = UDim2.new(0,220,0,360)}):Play()
    ...
    local btns = {
        ...
        {"Bounty Hunter", function() settings.bountyHunter = not settings.bountyHunter end, 208},
        {"Travel Island", function() for _,v in pairs(Workspace:GetChildren()) do if v.Name:lower():find(settings.selectedIsland:lower()) and v:IsA("Model") then tween(v:GetPivot().Position) break end end end, 248},
        {"Hop Server", function() TeleportService:Teleport(SEA_PLACE_IDS[SEA]) end, 288},
        {"Close", function() open = false TweenService:Create(main, TweenInfo.new(0.25), {Size = UDim2.new(0,0,0,0)}):Play() end, 328}
    }
...
Tabs.Main:AddToggle("AB", {Title = "Auto Boss (Live Only)", Default = false}):OnChanged(function(v) settings.autoBoss = v end)
...
Tabs.Fruit:AddToggle("ASF2", {Title = "Auto Spawn Fruit", Default = false}):OnChanged(function(v) settings.autoSpawnFruit = v end)
...
-- PVP
Tabs.PvP:AddToggle("BH", {Title = "Bounty Hunter", Default = false}):OnChanged(function(v) settings.bountyHunter = v end)

-- TRAVEL
Tabs.Travel:AddDropdown("ID", {Title = "Select Island", Values = ISLANDS[SEA], Default = ISLANDS[SEA][1], Multi = false}):OnChanged(function(v) settings.selectedIsland = v end)
...
Tabs.Misc:AddButton({Title = "Hop Server", Callback = function() TeleportService:Teleport(SEA_PLACE_IDS[SEA]) end})
Tabs.Misc:AddButton({Title = "Hop to Low Player Server", Callback = function()
...
-- // AUTO SPAWN FRUIT LOOP
coroutine.wrap(function()
    while task.wait(settings.autoSpawnInterval) do
        if settings.autoSpawnFruit then
            pcall(function()
                ...
                sendWebhook("AUTO SPAWN FRUIT", "Spawned "..settings.spawnedFruit.." Fruit at "..os.date("%H:%M:%S"))
            end)
        end
    end
end)()
...
sendWebhook("SCRIPT LOADED", "MantaxVly v7.0 loaded | Sea "..SEA.." | Level "..getLevel())
