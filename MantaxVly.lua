                    ...
                    if enemy.Name == "Dough King" and enemy:FindFirstChild("Humanoid") and enemy.Humanoid.Health > 0 then
                        if not settings.ignoreDoughKingItem then
                            ...
                            for _, item in pairs(Workspace:GetChildren()) do
                                if item.Name:lower():find("chalice") then
...
coroutine.wrap(function()
    while task.wait(1) do
        if settings.autoActivateObsHaki then
            pcall(function()
...
coroutine.wrap(function()
    while task.wait(1) do
        if settings.autoLeviathan then
            pcall(function()
                if settings.bribeLeviathan then
                ...
                if settings.autoFindLeviathan then
                    for _, levi in pairs(Workspace:GetChildren()) do
                        if levi.Name:lower():find("leviathan") then
                            tween(levi:GetPivot().Position)
                        end
                    end
                end
                if settings.autoKillLeviathan then
                    for _, enemy in pairs(Workspace.Enemies:GetChildren()) do
...
coroutine.wrap(function()
    while task.wait(1) do
        if settings.autoTrueFormRipIndra then
            pcall(function()
...
coroutine.wrap(function()
    while task.wait(1) do
        if settings.autoObservationV2 then
            pcall(function()
...
coroutine.wrap(function()
    while task.wait(settings.autoSpawnInterval) do
        if settings.autoRollFruit then
            pcall(function()
        ...
        if settings.autoUnstoreBelow1M then
            pcall(function()
                -- unstored logic
            end)
        end
...
coroutine.wrap(function()
    while task.wait(5) do
        if settings.autoSetSpawnPoint then
            pcall(function()
...
Tabs.Raid:AddToggle("IKR", {Title = "Instant Kill Islands 4-5", Default = false}):OnChanged(function(v) settings.instantKillRaid = v end)
